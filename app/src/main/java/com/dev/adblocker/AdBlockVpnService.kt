...
class AdBlockVpnService : VpnService() {
    ...
    companion object {
        const val ACTION_START = "com.dev.adblocker.START"
        const val ACTION_STOP  = "com.dev.adblocker.STOP"

        // Broadcast to inform MainActivity of state changes
        const val BROADCAST_STATUS  = "com.dev.adblocker.STATUS"
    ...
    private fun startVpn() {
        ...
        val builder = Builder()
            ...
            // Exclude the app itself from its own VPN tunnel so that blocklist
            // downloads and DNS forwarding sockets are not subject to routing
            // through the TUN interface, which can cause HTTP errors (e.g. 404)
            // when the app tries to reach external servers while the VPN is on.
