...
class MainActivity : AppCompatActivity() {
    ...
    private fun renderFilterSources() {
        ...
        for (src in FilterSources.ALL) {
            ...
            // Disable view-state save/restore: all rows share the same cbEnabled ID,
            // so Android's SparseArray collapses them to one entry and restores every
            // checkbox to the same wrong state after Activity recreation (theme change),
            // firing OnCheckedChangeListener and corrupting SharedPreferences.
