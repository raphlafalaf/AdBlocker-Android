...
class MainActivity : AppCompatActivity() {
    ...
    private fun renderFilterSources() {
        ...
        for (src in FilterSources.ALL) {
            ...
            // Disable view-state save/restore for dynamically created checkboxes.
            // All rows share the same cbEnabled ID, so Android's SparseArray would
            // collapse them to one entry and restore every checkbox to the same (wrong)
            // state after Activity recreation (e.g. on theme change). That fires
            // OnCheckedChangeListener and corrupts SharedPreferences. State is managed
            // solely through PrefsManager, so view-level persistence is not needed.
